  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/signup-requests/verify-email?token=${encodeURIComponent(token)}`);
        const body = await res.json().catch(() => ({}));

        if (!res.ok) throw new Error(body.message || 'Verification failed');

        setStatus('success');
        setMessage('Email verified successfully! Your signup request has been submitted and is pending coordinator approval.');
